 ...
 */
export async function autoExtractMemories(engine, sessionId, provider, condense = false) {
    try {
        const history = engine.getHistory();
        if (condense)
            log.info('记忆总结：开始 LLM 提炼', { sessionId, caller: 'memory-pipeline' });
        await runMemoryPipeline(history, {
            condense,
            provider: condense ? provider : undefined,
            sessionId,
        });
        if (condense)
            log.info('记忆总结：LLM 提炼完成', { sessionId, caller: 'memory-pipeline' });
    }
    catch {
        // 静默失败，不影响主流程
    }
 ...
 */
export async function autoDistillSkill(engine, provider) {
    try {
        const history = engine.getHistory();
        // 统计工具调用次数
        let toolCallCount = 0;
...
${condensed}`;
        let raw = '';
        for await (const chunk of provider.stream([{ role: 'user', content: distillPrompt }], [], '你是一个 skill 提炼助手，只输出 null 或 JSON，不输出任何其他内容。', 2000)) {
