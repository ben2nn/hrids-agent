...
// 创建内置命令集
export function createBuiltinCommands(apiKey, model) {
    return [
        {
            name: 'clear',
            description: '清空对话历史',
        ...
        },
        {
            ...
            async execute(args, ctx) {
                ...
                if (list.length === 0)
                    return { type: 'message', text: '没有历史会话。' };
        ...
        },
        {
            ...
            async execute(args, ctx) {
                ...
                if (!input)
                    return { type: 'message', text: '用法: /resume <会话ID 或 序号>\n输入 /sessions 查看历史会话列表' };
                // 支持序号（如 /resume 2）
                let targetId = input;
                ...
                if (!isNaN(idx) && idx > 0) {
                    ...
                    if (!target)
                        return { type: 'message', text: `序号 ${idx} 超出范围，共 ${list.length} 条会话` };
                    targetId = target.id;
                }
                const ok = ctx.switchSession(targetId);
                if (!ok)
                    return { type: 'message', text: `未找到会话: ${targetId}` };
