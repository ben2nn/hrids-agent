...
export function App({ engine, commands, sessionId: initialSessionId, onModelChange, currentModel, providerName, getProviderName }) {
    const { exit } = useApp();
    ...
    const [askUserPrompt, setAskUserPrompt] = useState(null); // ask_user 等待时的提示文字
    ...
    const updateMsg = useCallback((id, updater, fallback) => {
        setMsgs(prev => {
    ...
    const bufferToolLog = useCallback((line) => {
        ...
        if (toolLogBufRef.current.length >= 20) {
            if (toolLogFlushRef.current)
                clearTimeout(toolLogFlushRef.current);
            flushToolLog();
        }
    ...
    const runEngine = useCallback(async (prompt, displayAs) => {
        if (displayAs !== undefined)
            push({ role: 'system', text: displayAs, color: displayAs.startsWith('⏰') ? 'yellow' : undefined });
        setLoading(true);
            ...
            for await (const ev of engine.send(prompt)) {
                switch (ev.type) {
                    ...
                    case 'permission_denied':
                        ...
                        break;
                    case 'usage':
                        setCostInfo({ inputTokens: ev.inputTokens, outputTokens: ev.outputTokens, costUsd: ev.costUsd });
                        break;
                    case 'compact_start':
                    ...
                    case 'done':
                        if (assistantText) {
                            setStreamBuf('');
                            push({ role: 'assistant', text: assistantText });
                            assistantText = '';
                        }
        ...
        if (next) {
            void runEngine(`[定时任务触发] ${next.description}\n\n${next.task}`, `⏰ 定时任务触发: ${next.description}`);
        }
    }, [engine, push]);
    ...
    const cmdCtx = {
        ...
        setModel: (m) => { modelRef.current = m; onModelChange(m); },
    ...
    const handleSubmit = useCallback(async (value) => {
        ...
        if (parsed) {
            ...
            if (result.type === 'message') {
                push({ role: 'system', text: result.text });
                return;
            }
            if (result.type === 'noop')
                return;
    ...
    return (_jsxs(Box, { flexDirection: "column", paddingX: 1, paddingY: 0, children: [_jsx(Box, { flexDirection: "column", marginBottom: 1, children: msgs.map((m, i) => (_jsx(Box, { marginBottom: 0, children: _jsxs(Text, { color: m.color ?? ROLE_COLOR[m.role], children: [ROLE_PREFIX[m.role], m.text] }) }, m.id ?? i))) }), loading && toolProgress && (_jsx(Box, { marginBottom: 1, children: _jsx(Text, { color: "cyan", dimColor: true, children: toolProgress }) })), loading && streamBuf && (_jsx(Box, { marginBottom: 1, children: _jsxs(Text, { color: "white", children: ["\u2726 ", streamBuf] }) })), _jsx(Box, { marginBottom: 0, children: askUserPrompt
                    ? (_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: "yellow", children: askUserPrompt }), _jsxs(Box, { children: [_jsx(Text, { color: "cyan", children: '› ' }), _jsx(TextInput, { value: input, onChange: setInput, onSubmit: handleSubmit, placeholder: "\u8F93\u5165\u56DE\u7B54..." })] })] }))
                    : loading
                        ? _jsx(Text, { color: "yellow", dimColor: true, children: "\u25B8 \u601D\u8003\u4E2D..." })
                        : (_jsxs(Box, { children: [_jsx(Text, { color: "cyan", children: '› ' }), _jsx(TextInput, { value: input, onChange: setInput, onSubmit: handleSubmit, placeholder: "\u8F93\u5165\u6D88\u606F\u6216 /\u547D\u4EE4..." })] })) }), _jsx(Box, { marginTop: 0, children: _jsxs(Text, { dimColor: true, children: [displayProvider, "  ", modelRef.current, costInfo && ` 输入 ${costInfo.inputTokens} / 输出 ${costInfo.outputTokens} tokens  累计 ${costInfo.costUsd.toFixed(4)}`, loading ? '  Ctrl+C 中断' : '  Ctrl+C 退出'] }) })] }));
}
