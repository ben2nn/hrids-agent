...
- 保持工作流一致性`,
    ...
    async execute(input) {
        ...
        const loadErrors = [];
        const registry = buildSkillRegistry(getGlobalCwd(), (source, skillName, err) => { loadErrors.push(`[${source}] ${skillName}: ${String(err)}`); });
...
export const SkillListTool = {
    ...
    async execute() {
        ensureBundledSkillsRegistered();
        const loadErrors = [];
        const registry = buildSkillRegistry(getGlobalCwd(), (source, skillName, err) => {
...
const saveSchema = z.object({
    ...
    scope: z.enum(['user', 'project']).default('user').describe('user = 保存到 ~/.hrids-agent/skills/（跨项目可用）；project = 保存到 .agent/skills/（仅当前项目）'),
});
export const SkillSaveTool = {
    name: 'skill_save',
