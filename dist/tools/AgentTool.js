...
const inputSchema = z.object({
    ...
    allowed_tools: z.array(z.string()).optional().describe('允许使用的工具列表，默认全部允许'),
    isolated: z.boolean().optional().describe('是否使用独立的临时工作目录（worktree 隔离），默认 false。并行任务建议设为 true 避免互相干扰'),
});
const SUB_AGENT_SYSTEM_PROMPT = `你是一个专注的子智能体，负责完成分配给你的具体任务。
- 专注于完成任务，不要偏离主题
- 完成后输出清晰的结果摘要
- 如果遇到无法解决的问题，说明原因并返回已完成的部分`;
export function createAgentTool(apiKey, model) {
    return {
        name: 'agent',
