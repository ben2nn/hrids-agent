...
export function resolveDecision(answer) {
...
- 可以通过 ask_user 解决的简单信息收集`,
    ...
    async execute(input) {
        ...
        if (input.impact)
            sections.push(`\n🎯 影响范围: ${input.impact}`);
        if (input.deadline)
            sections.push(`⏰ 紧迫性: ${input.deadline}`);
        sections.push(`\n📖 背景\n${input.context}`);
        sections.push(`\n🔀 可选方案\n${optionLines}`);
...
function parseDecisionAnswer(answer, options) {
    // 数字选项
    if (/^\d+$/.test(answer)) {
