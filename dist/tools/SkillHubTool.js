 ...
 */
async function fetchSkillHubPage(url) {
...
function parseSkillListFromHtml(html) {
    const results = [];
    const seen = new Set();
...
function extractSkillContent(html, skillId, skillTitle) {
...
function buildSkillMd(id, title, description, body) {
...
export const SkillHubConfigTool = {
...
- SKILLHUB_API_BASE   SkillHub API 基础地址（默认 https://skillhub.cn）`,
    ...
    async execute(input) {
        ...
        const updates = {};
        if (input.skillhub_url)
            updates['SKILLHUB_URL'] = input.skillhub_url;
        if (input.skillhub_api_base)
            updates['SKILLHUB_API_BASE'] = input.skillhub_api_base;
...
async function fetchRemoteSkillInfo(slug) {
    try {
        ...
        const exact = data.results.find((item) => {
            return typeof item === 'object' && item !== null && item['slug'] === slug;
        });
        if (!exact)
            return null;
        return {
...
async function downloadAndExtractSkill(slug, targetDir, force) {
    ...
    try {
        ...
        if (process.platform === 'win32') {
            execSync(`powershell -NoProfile -NonInteractive -Command "Expand-Archive -Path '${tmpZip}' -DestinationPath '${tmpDir}' -Force"`, { timeout: 30000 });
        }
        else {
...
export const SkillHubSearchTool = {
...
const installSchema = z.object({
    ...
    scope: z.enum(['user', 'project']).default('user').describe('user（默认）= 安装到 ~/.hrids-agent/skills/；project = 安装到 .agent/skills/'),
    force: z.boolean().default(false).describe('是否覆盖已存在的同名技能'),
});
export const SkillHubInstallTool = {
...
export const SkillHubSetupTool = {
    name: 'skillhub_setup',
...
export const SkillHubRecommendTool = {
    ...
    async execute(input) {
        ...
        const lines = [
            ...
            '',
            ...results.slice(0, 6).map((r, i) => `${i + 1}. **${r.title}**（关键词：${r.relevance}）\n   安装: skillhub_install skill_id="${r.id}"`),
            '',
            '如需安装，直接告诉我"安装 <技能名>"。',
        ];
        return { type: 'success', output: lines.join('\n') };
...
async function tryInstallViaCli(skillId, scope, force) {
    ...
    try {
        ...
        if (scope === 'user' && existsSync(cliDefaultDir)) {
            ...
            try {
                rmSync(cliDefaultDir, { recursive: true, force: true });
            }
            catch { /* 忽略 */ }
        }
...
async function installOnUnix(scriptUrl) {
    const { execSync } = await import('child_process');
...
async function installOnWindows(scriptUrl) {
    const { execSync } = await import('child_process');
    ...
    try {
        execSync(`powershell -NoProfile -NonInteractive -Command "Invoke-WebRequest -Uri '${psScriptUrl}' -OutFile '${tmpScript}'"`, { timeout: 30000, encoding: 'utf-8' });
        execSync(`powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "${tmpScript}"`, { timeout: 120000, encoding: 'utf-8', stdio: 'pipe' });
        try {
            execSync(`del /f "${tmpScript}"`);
        }
        catch { /* 忽略 */ }
...
function extractKeywords(task) {
    ...
    for (const w of chineseWords) {
        if (!keywords.includes(w) && keywords.length < 5)
            keywords.push(w);
    }
    const englishWords = task.match(/[a-zA-Z]{3,}/g) ?? [];
    for (const w of englishWords) {
        if (!keywords.includes(w.toLowerCase()) && keywords.length < 5)
...
function loadLockfile(skillsDir) {
...
function saveLockfile(skillsDir, lock) {
...
function extractUpdateUrl(config) {
...
const listSchema = z.object({
    scope: z.enum(['user', 'project', 'all']).default('all').describe('user = ~/.hrids-agent/skills/；project = .agent/skills/；all（默认）= 两者都列出'),
});
export const SkillHubListTool = {
    name: 'skillhub_list',
...
const uninstallSchema = z.object({
    ...
    scope: z.enum(['user', 'project']).default('user').describe('user（默认）= 从 ~/.hrids-agent/skills/ 卸载；project = 从 .agent/skills/ 卸载'),
});
export const SkillHubUninstallTool = {
...
const upgradeSchema = z.object({
    ...
    scope: z.enum(['user', 'project']).default('user').describe('user（默认）= ~/.hrids-agent/skills/；project = .agent/skills/'),
    check_only: z.boolean().default(false).describe('仅检查可用更新，不实际安装'),
});
export const SkillHubUpgradeTool = {
