...
import { runWithCwd } from '../tools/BashTool.js';
...
export class SessionManager {
    ...
    async createSession(req) {
        ...
        const provider = (req.model || req.provider || req.apiKey)
            ? createProvider({
                ...
                provider: req.provider ?? agentConfig.provider,
            })
        ...
        const session = {
            ...
            engine: null, // 下方赋值
            provider,
            permissions: null, // 下方赋值
            subscribers: new Set(),
            idleTimer: null,
            pendingPermissions: new Map(),
            replayBuffer: [],
        };
        const permissions = new PermissionManager(permMode, async (permReq) => {
        ...
        session.engine.onBeforeCompact = async (summary) => {
    ...
    async gracefulShutdown(timeoutMs = 10000) {
    ...
    async sendCronReminder(sessionId, cronJob) {
        const session = this.sessions.get(sessionId);
        if (!session)
        ...
        try {
            ...
            session.engine.setHistory([
                ...
                {
                    role: 'assistant',
                    content: [{ type: 'text', text: cronJob.task, requestId }],
                    requestId,
                },
            ]);
            saveSession(sessionId, session.engine.getHistory(), session.info.model, session.info.cwd);
    ...
    // 执行用户消息，流式广播事件
    async runMessage(sessionId, content, attachments, cronJob) {
        const session = this.sessions.get(sessionId);
        if (!session)
            throw new Error(`会话不存在: ${sessionId}`);
        if (session.info.status === 'busy')
            throw new Error('会话正忙，请等待当前任务完成');
        // 在会话自己的 cwd 上下文中运行，与其他并发会话完全隔离
        return runWithCwd(session.info.cwd, () => this._runMessageInContext(session, sessionId, content, attachments, cronJob));
    }
    // 实际执行逻辑（已在正确的 cwd 上下文中）
    async _runMessageInContext(session, sessionId, content, attachments, cronJob) {
...
用户确认计划后，会将模式切换为 ask 或 auto，届时写操作将可用。`;
        ...
        if (hitTurnLimit) {
            ...
            for (let i = history.length - 1; i >= 0; i--) {
                ...
                if (msg.role !== 'assistant' || !Array.isArray(msg.content))
                    continue;
                const todoCall = msg.content
                    .find(b => b.type === 'tool_use' && b.name === 'todo_write');
                if (todoCall) {
                    const todos = todoCall.input?.todos ?? [];
    ...
    // 处理客户端发来的控制消息
    handleClientMessage(sessionId, raw) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
        let msg;
...
function toClientMessage(ev, requestId) {
    switch (ev.type) {
        case 'text_delta':
...
function isVisionIntent(message) {
...
function extractRecentImagesFromHistory(history, cwd) {
