...
// 动态重要性评分（1-5）
function calcImportance(type, text, confidence) {
    let score = TYPE_BASE_SCORE[type];
    // 内容越详细越重要
    if (text.length > 200)
        score += 0.5;
    // 包含具体版本号、数字、技术名称
    if (/v\d+\.\d+|\d{4}|[A-Z][a-z]+[A-Z]/.test(text))
        score += 0.5;
    // 高置信度加分
    if (confidence >= 0.7)
        score += 0.5;
...
export function extractMemories(text, minConfidence = 0.3) {
    ...
    for (const seg of segments) {
        ...
        let bestType = Object.entries(scores).sort((a, b) => b[1] - a[1])[0][0];
...
function splitIntoSegments(text) {
    ...
    for (const line of lines) {
        if (turnPatterns.some(p => p.test(line.trim())))
            turnCount++;
    }
    if (turnCount >= 3) {
...
function splitByTurns(lines, patterns) {
    const segments = [];
    let current = [];
    for (const line of lines) {
