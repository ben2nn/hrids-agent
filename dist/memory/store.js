...
export class MemoryStore {
    ...
    _restoreDim() {
        const saved = this.db.prepare("SELECT value FROM vec_config WHERE key='dim'").get();
        if (saved) {
    ...
    addMemory(mem) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, mem.content, mem.type, mem.wing ?? 'general', mem.room ?? 'general', JSON.stringify(mem.tags ?? []), mem.importance ?? 3, createdAt, mem.sourceSession ?? null);
        const rowid = Number(result.lastInsertRowid);
        // 异步生成 embedding 并写入向量后端
        void this._embedAndInsertVec(id, mem.content);
        return { ...mem, id, createdAt };
    }
    async _embedAndInsertVec(id, content) {
        try {
     ...
     * 用于写入前判断是否已有重复内容
     */
    async findSimilar(content, threshold = 0.85, topK = 3) {
        if (this._dim === null)
            return [];
        try {
            const provider = getEmbeddingProvider();
    ...
    listMemories(opts = {}) {
        ...
        return this.db.prepare(sql).all(...params).map(r => this._rowToMemory(r));
    }
    /**
            ...
            try {
                ...
                if (queryVec.length === this._dim) {
                    ...
                    for (const { id, score } of hits) {
                        ...
                        if (!mem)
                            continue;
                        if (opts.wing && mem.wing !== opts.wing)
                            continue;
                        if (opts.room && mem.room !== opts.room)
                            continue;
    ...
    /** 关键词匹配降级（无向量时使用） */
    _keywordSearch(query, wing, room, topK = 5) {
    ...
    getEssentialStory(wing, maxItems = 15, maxChars = 3200) {
    ...
    addTriple(subject, predicate, object, opts = {}) {
        ...
        const existing = this.db.prepare('SELECT id FROM triples WHERE subject=? AND predicate=? AND object=? AND valid_to IS NULL').get(subId, pred, objId);
        if (existing)
            return this.getTriple(existing.id);
    ...
    invalidateTriple(subject, predicate, object, endedAt) {
        const subId = subject.toLowerCase().replace(/\s+/g, '_');
        const objId = object.toLowerCase().replace(/\s+/g, '_');
        const pred = predicate.toLowerCase().replace(/\s+/g, '_');
        const ended = endedAt ?? new Date().toISOString().slice(0, 10);
        this.db.prepare('UPDATE triples SET valid_to=? WHERE subject=? AND predicate=? AND object=? AND valid_to IS NULL').run(ended, subId, pred, objId);
    }
    queryEntity(name, opts = {}) {
        ...
        if (dir === 'outgoing' || dir === 'both') {
            const rows = this.db.prepare(`SELECT * FROM triples WHERE subject=? ${timeFilter} ORDER BY created_at DESC`).all(eid, ...timeParams);
            results.push(...rows.map(r => this._rowToTriple(r)));
        }
        if (dir === 'incoming' || dir === 'both') {
            const rows = this.db.prepare(`SELECT * FROM triples WHERE object=? ${timeFilter} ORDER BY created_at DESC`).all(eid, ...timeParams);
            results.push(...rows.map(r => this._rowToTriple(r)));
        }
        return results;
    }
    ...
    async stats() {
        ...
        const triples = this.db.prepare('SELECT COUNT(*) as c FROM triples WHERE valid_to IS NULL').get().c;
        const wings = this.db.prepare('SELECT DISTINCT wing FROM memories').all();
        let vecCount = 0;
        if (this._dim !== null) {
