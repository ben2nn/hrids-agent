 ...
 */
export async function runMemoryPipeline(messages, opts = {}) {
    const { condense = false, provider, dedupThreshold = 0.85, minConfidence = 0.4, sessionId, } = opts;
    const result = { saved: 0, skipped: 0, condensed: 0 };
    // ① 提取：正则初筛，过滤低置信度
    const extracted = extractFromConversation(messages).filter(m => m.confidence >= minConfidence);
    if (extracted.length === 0)
        return result;
    const store = getMemoryStore();
    // ② LLM 批量提炼：一次调用压缩所有需要提炼的条目
    let condensedContents = extracted.map(() => null);
    if (condense && provider) {
        ...
        if (needCondense.length > 0) {
            const batchResults = await condenseBatchWithLLM(needCondense.map(({ content }) => content), provider);
...
async function condenseBatchWithLLM(contents, provider) {
...
${numbered}`;
    try {
        let raw = '';
        for await (const chunk of provider.stream([{ role: 'user', content: prompt }], [], '你是记忆提炼助手，严格按编号格式输出压缩结果，不输出任何其他内容。', Math.min(200 * contents.length, 2000))) {
            if (chunk.type === 'text_delta' && chunk.delta)
                raw += chunk.delta;
        }
        // 解析输出：按 [N] 分割
        const results = contents.map(() => null);
