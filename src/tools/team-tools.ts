...
import { getCurrentAgentName } from '../coordinator/agent-context.js'
import { getCurrentSessionId } from '../core/session-context.js'
...
export const TeamCreateTool: ToolDef<typeof teamCreateSchema> = {
  name: 'team_create',
...
export const TeamDeleteTool: ToolDef<typeof teamDeleteSchema> = {
  name: 'team_delete',
...
export const AgentSpawnTool: ToolDef<typeof agentSpawnSchema> = {
  name: 'agent_spawn',
...
export const TeamStatusTool: ToolDef<typeof teamStatusSchema> = {
  name: 'team_status',
  ...
  async execute(input) {
    ...
    const team = mgr.getTeam(input.team)
    if (!team) return { type: 'error', message: `团队 "${input.team}" 不存在` }

    const tasks = team.pool.listTasks()
    if (tasks.length === 0) return { type: 'success', output: '团队中没有任务' }
...
export const TeamWaitTool: ToolDef<typeof teamWaitSchema> = {
  name: 'team_wait',
...
export const SendMessageTool: ToolDef<typeof sendMessageSchema> = {
  name: 'send_message',
...
export const ReceiveMessageTool: ToolDef<typeof receiveMessageSchema> = {
