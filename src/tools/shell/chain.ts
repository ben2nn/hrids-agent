/**
 * 命令链解析与执行 —— 不调用真实 shell，通过管道串联进程
 *
 * 支持：|, ||, &&, ;, >, >>, <, 2>, 2>>, 2>&1, &>
 * 不支持：&（后台）, <<（heredoc）, $(), (...), $VAR, glob 展开
 *
 * 设计移植自 DeepSeek-Reasonix shell-chain.ts
 */
...
export interface Redirect {
  kind: RedirectKind
  /** 文件路径（相对于 chain 的 cwd）；2>&1 时为空字符串 */
  target: string
}
...
/**
 * 按空白边界拆分链式操作符
 * 操作符只在 token 开头生效，所以 --flag=1&2 保持字面量
 */
function splitOnChainOps(cmd: string): { segs: string[]; ops: ChainOp[] } {
  ...
  while (i < cmd.length) {
    ...
    if (atTokenStart) {
      ...
      let opLen = 0
      const next = cmd[i + 1]
      if (ch === '|' && next === '|') { op = '||'; opLen = 2 }
      else if (ch === '&' && next === '&') { op = '&&'; opLen = 2 }
      else if (ch === '|') { op = '|'; opLen = 1 }
      else if (ch === ';') { op = ';'; opLen = 1 }
      if (op !== null) {
    ...
    if (cur.length === 0 && !curHasContent) {
      ...
      if (remaining.startsWith('2>&1')) matched = { op: '2>&1', len: 4 }
        ...
        throw new UnsupportedSyntaxError(
          'shell operator "<<" is not supported — heredoc is not implemented; pass input via a "<" file or the binary\'s --input flag',
        )
      } else if (remaining.startsWith('<')) matched = { op: '<', len: 1 }
      if (matched) {
        if (pending !== null) {
...
function validateRedirectFds(redirects: readonly Redirect[]): void {
  ...
  for (const r of redirects) {
    ...
    else if (r.kind === '&>') { stdout++; stderr++ }
  }
  if (stdin > 1) throw new UnsupportedSyntaxError('multiple `<` stdin redirects in one segment')
  if (stdout > 1) throw new UnsupportedSyntaxError('multiple stdout redirects in one segment (`>` / `>>` / `&>` conflict)')
  if (stderr > 1) throw new UnsupportedSyntaxError('multiple stderr redirects in one segment (`2>` / `2>>` / `&>` / `2>&1` conflict)')
}
...
/**
 * 解析命令链
 * @returns CommandChain（有链式操作符或重定向时）或 null（普通单命令）
 */
export function parseCommandChain(cmd: string): CommandChain | null {
  ...
  // 拒绝 cd 出现在链中 — 执行器无法在 segment 间传递 cwd 变更
  for (const seg of segments) {
    const cmdName = seg.argv[0] ?? ''
    if (cmdName.toLowerCase() === 'cd') {
      throw new UnsupportedSyntaxError(
        'cd in parsed command chains does not change cwd for later segments. Use a command-native cwd flag instead, such as `npm --prefix <dir>`, `git -C <dir>`, or `cargo -C <dir>`.',
      )
    }
  }
