...
export interface RetryOptions {
  ...
  retryIf?: (err: unknown) => boolean  // 自定义判断是否可重试，默认重试网络/限流错误
}
...
function defaultRetryIf(err: unknown): boolean {
  if (err instanceof Error) {
    ...
    if (msg.includes('network') || msg.includes('timeout') || msg.includes('超时')) return true
    // HTTP 429 限流
    ...
    if (msg.includes('403') || msg.includes('404') || msg.includes('400')) return true
    // HTTP 5xx 服务端错误
    if (msg.includes('500') || msg.includes('502') || msg.includes('503') || msg.includes('504')) return true
    // 流式读取超时
    if (msg.includes('流式读取超时')) return true
  }
  return false
}
