...
**豁免情况（不需要建立任务计划，直接执行后输出结果）**：
- 单步信息检索：查找/搜索/推荐/列举某类内容（如"查找XX参考"、"搜索XX"、"推荐XX"）
- 简单问答：直接回答即可，无需调用多个工具
- 纯只读查询：一次工具调用即可完成的任务

**状态 1：无任务计划（todo_read 返回空 / 从未调用过）**
→ 先判断是否属于豁免情况（见上），再决定是否建立计划：
  - 豁免情况 → 直接执行，完成后输出结果，不调用任何任务工具
  - 低不确定性（直接执行）：单文件修改、局部变更、意图明确且需要多步骤
